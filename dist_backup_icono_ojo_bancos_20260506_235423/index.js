...
__export(db_exports, {
  ...
  limpiarHistorialEliminados: () => limpiarHistorialEliminados,
...
async function limpiarHistorialEliminados() {
  const db = await getDb();
  if (!db) throw new Error("DB no disponible");
...
function groupByBankName(value) {
  return cleanText(value, "SIN BANCO").toUpperCase();
}
async function generarPDFPagosBancos(periodoId, options = {}) {
  const periodo = await getPeriodoById(periodoId);
  if (!periodo) throw new Error("Per\xEDodo no encontrado");
  const empleados2 = await getEmpleados(periodoId);
  const diasPeriodo = await getDiasPeriodo(periodoId);
  const diasSeleccionados = await getPeriodoDiasSeleccionados(periodoId);
  ...
  const rows = empleados2.filter((emp) => departamentoId == null || Number(emp.departamentoId) === departamentoId).filter((emp) => !bancoFiltro || bancoFiltro === "TODOS" || groupByBankName(emp.banco) === bancoFiltro).map((emp) => {
    const salario = parseFloat(String(emp.salarioMensual ?? "0")) || 0;
    const bonos = parseFloat(String(emp.bonos ?? "0")) || 0;
    ...
    const diasPagables = emp.diasLaborados !== null && emp.diasLaborados !== void 0 && String(emp.diasLaborados).trim() !== "" ? Math.max(0, Math.round(parseFloat(String(emp.diasLaborados)) || diasBase)) : diasBase;
  ...
  }).filter((row) => row.importe > 0).sort((a, b) => a.banco.localeCompare(b.banco, "es") || a.nombre.localeCompare(b.nombre, "es"));
  return new Promise((resolve3, reject) => {
    ...
    const sectionH = 20;
    const col = [
      { key: "nombre", label: "Nombre", x: 18, w: 210, align: "left" },
      { key: "importe", label: "", x: 228, w: 86, align: "right" },
      { key: "numeroCuenta", label: "No de cuenta", x: 314, w: 104, align: "center" },
      { key: "tarjeta", label: "Tarjeta", x: 418, w: 132, align: "center" },
      { key: "clabeInterbancaria", label: "Clabe interbancaria", x: 550, w: 166, align: "center" },
      { key: "banco", label: "Banco", x: 716, w: 108, align: "center" }
    ];
    function drawCell(x, y, w, h, text2, opts = {}) {
      const previousX = doc.x;
      const previousY = doc.y;
      if (opts.fill) doc.rect(x, y, w, h).fill(opts.fill);
      doc.rect(x, y, w, h).strokeColor("#111111").lineWidth(0.5).stroke();
      doc.fillColor("#111111").font(opts.bold ? "Helvetica-Bold" : "Helvetica").fontSize(opts.size ?? 7.2).text(text2, x + 4, y + 4.2, { width: w - 8, align: opts.align ?? "left", lineBreak: false, ellipsis: true });
      doc.x = previousX;
      doc.y = previousY;
    }
    ...
    doc.font("Helvetica").fontSize(8).text(`Per\xEDodo: ${periodo.nombre}`, page.left, page.top + 15, { width: page.right - page.left, align: "center" });
    doc.text(`Generado: ${(/* @__PURE__ */ new Date()).toLocaleDateString("es-MX")}`, page.left, page.top + 27, { width: page.right - page.left, align: "center" });
    doc.y = 62;
    drawHeader(doc.y);
    doc.y += headerH;
    if (rows.length === 0) {
      drawCell(page.left, doc.y, page.right - page.left, 32, "No hay pagos a bancos que coincidan con el per\xEDodo y filtros seleccionados.", { align: "center", size: 9 });
      doc.end();
      return;
    }
    const grupos = /* @__PURE__ */ new Map();
    for (const row of rows) {
      if (!grupos.has(row.banco)) grupos.set(row.banco, []);
      grupos.get(row.banco).push(row);
    }
    let totalGeneral = 0;
    for (const [banco, bancoRows] of grupos.entries()) {
      ...
      doc.y += sectionH;
      const subtotal = bancoRows.reduce((sum, row) => sum + row.importe, 0);
      totalGeneral += subtotal;
      bancoRows.forEach((row, index) => {
