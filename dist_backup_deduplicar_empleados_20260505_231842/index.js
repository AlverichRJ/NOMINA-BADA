...
async function ensureAppSchema() {
  ...
  try {
    ...
    const [empleadosExistentes] = await rawConn.execute(
      `SELECT id, nombre FROM empleados`
    );
    const empleadosPorClave = /* @__PURE__ */ new Map();
    for (const emp of empleadosExistentes ?? []) {
      empleadosPorClave.set(normalizarClaveEmpleado(emp.nombre), emp);
    }
    for (const emp of CATALOGO_NOMINA_EMPLEADOS) {
      ...
      const existente = empleadosPorClave.get(normalizarClaveEmpleado(emp.nombre));
      if (existente) {
        await rawConn.execute(
          `UPDATE empleados
